...
import BpkColoredCalendarDate from './BpkColoredCalendarDate';

const createNodeMock = () => ({
  focus: () => null,
});

describe('BpkColoredCalendarDate', () => {
  it('should render correctly', () => {
    const tree = renderer
      .create(<BpkColoredCalendarDate date={new Date(2010, 1, 15)} />)
      .toJSON();
  ...
  it('should render a disabled button', () => {
    const tree = renderer
      .create(<BpkColoredCalendarDate date={new Date(2010, 1, 15)} disabled />)
      .toJSON();
    expect(tree).toMatchSnapshot();
  });
  ...
  it('should render a focused button with tabIndex', () => {
    const tree = renderer
      .create(<BpkColoredCalendarDate date={new Date(2010, 1, 15)} focused />, {
        createNodeMock,
      })
      .toJSON();
    expect(tree).toMatchSnapshot();
