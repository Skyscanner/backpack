...
function generateFile(iconSets, smIcons, lgIcons) {
  ...
  // Build imports using consumer-facing package paths so Figma displays
  // the correct import for end users (e.g. @skyscanner/backpack-web/bpk-component-icon/sm/flight)
  const allImports = [];
  for (const e of entries) {
    if (e.lgFile) {
      const pascal = kebabToPascal(e.lgFile);
      allImports.push(
        `import BpkLarge${pascal}Icon from '@skyscanner/backpack-web/bpk-component-icon/lg/${e.lgFile}';`,
      );
    }
    if (e.smFile) {
      const pascal = kebabToPascal(e.smFile);
      allImports.push(
        `import BpkSmall${pascal}Icon from '@skyscanner/backpack-web/bpk-component-icon/sm/${e.smFile}';`,
      );
    }
  }
  allImports.sort((a, b) => {
