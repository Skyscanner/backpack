...
import BpkSkeleton, { SKELETON_TYPES, SIZE_TYPES, IMAGE_SKELETON_STYLE } from './BpkSkeleton';

describe('BpkSkeleton', () => {
  it('renders correctly with default props', () => {
    const { container } = render(<BpkSkeleton type={SKELETON_TYPES.image} ariaLabel="Test Skeleton" />);
    expect(container.firstChild).toHaveClass('bpk-skeleton__image--default');
  });

  it('applies correct class names for specified size', () => {
    const { container } = render(<BpkSkeleton type={SKELETON_TYPES.bodyText} size={SIZE_TYPES.large} ariaLabel="Test Skeleton" />);
  ...
  it('applies correct class names for custom size', () => {
    ...
    const { container } = render(<BpkSkeleton type={SKELETON_TYPES.circle} size={customSize} ariaLabel="Test Skeleton" />);
  ...
  it('applies correct class names for image style', () => {
    const { container } = render(<BpkSkeleton type={SKELETON_TYPES.image} ariaLabel="Test Skeleton" style={IMAGE_SKELETON_STYLE.rounded} />);
