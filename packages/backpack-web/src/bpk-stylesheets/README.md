...
```js
import '@skyscanner-internal/backpack-web/bpk-stylesheets/base';
import '@skyscanner-internal/backpack-web/bpk-stylesheets/base.css';
```

By default font rendering is not included if you need to include Skyscanner Relative in your styles, import it using the following:

```js
import '@skyscanner-internal/backpack-web/bpk-stylesheets/font.css';
```
...
```js
import '@skyscanner-internal/backpack-web/bpk-stylesheets/larken.css';
```

## Design tokens (CSS variables)

`base.css` ships the design token CSS variables generated by [`token-sync`](https://github.com/Skyscanner/design-system/tree/main/token-sync). They are emitted as:
...
`nx run backpack-web:build-stylesheets` declares `copy-normalize-css` and `copy-token-css` as dependencies in `project.json`, so both files are copied automatically before the webpack build runs. `pnpm run build` also works (it runs `nx run-many -t build`). For `nx run backpack-web:build-sass`, run `nx run backpack-web:copy-normalize-css` first if `normalize.scss` isn't already vendored.
