...
| `BpkContextMenu.Item` | `Menu.Item` | Selectable menu row. Supports `startIcon` (left), `endIcon` (right), `destructive` variant, and an optional `onSelect` for item-specific handlers (fires for both pointer and keyboard activation). |
