...
export type Props = {
  ...
  // @ts-expect-error TS(7010) FIXME: 'Element', which lacks return-type annotation, imp... Remove this comment to see the full error message
  }) => Element<any>,
  /**
...
// @ts-expect-error TS(2693) FIXME: 'PropsWithDefault' only refers to a type, but is b... Remove this comment to see the full error message
  ...
  class WithInfiniteScroll extends Component<Props, State> {
    ...
    // @ts-expect-error TS(2304) FIXME: Cannot find name 'handleKeyPress'.
    ...
    constructor(props: Props) {
      // @ts-expect-error TS(2337) FIXME: Super calls are not permitted outside constructors... Remove this comment to see the full error message
      super(props);

      this.state = {
        // @ts-expect-error TS(2304) FIXME: Cannot find name 'index'.
        index: 0,
        elementsToRender: [],
      ...
      const thresholds = {
        // @ts-expect-error TS(2304) FIXME: Cannot find name 'small'.
        small: 0.01,
        half: 0.5,
      ...
      this.observer = new IntersectionObserver(this.handleIntersection, {
        // @ts-expect-error TS(2304) FIXME: Cannot find name 'threshold'.
    ...
    componentDidMount() {
      ...
      // @ts-expect-error TS(7006) FIXME: Parameter 'newState' implicitly has an 'any' type.
      }).then((newState) => {
        // @ts-expect-error TS(2532) FIXME: Object is possibly 'undefined'.
    ...
    componentDidUpdate(prevProps) {
      ...
      if (this.props.dataSource !== prevProps.dataSource) {
        // @ts-expect-error TS(2304) FIXME: Cannot find name 'prevProps'.
    ...
    setStateAfterDsUpdate(newState: State) {
      ...
      // @ts-expect-error TS(2609) FIXME: JSX spread child must be an array type.
      this.setState({
        // @ts-expect-error TS(2304) FIXME: Cannot find name 'newState'.
        ...newState,
        // @ts-expect-error TS(2304) FIXME: Cannot find name 'elementsToRender'.
    ...
    fetchItems(config): Promise<$Shape<State>> {
      ...
      return this.props.dataSource
        ...
        .then((nextElements) => {
          // @ts-expect-error TS(2304) FIXME: Cannot find name 'let'.
          let result = {
            // @ts-expect-error TS(2304) FIXME: Cannot find name 'isListFinished'.
            isListFinished: true,
          };
          ...
          if (onScrollFinished && result.isListFinished) {
            // @ts-expect-error TS(2304) FIXME: Cannot find name 'onScrollFinished'.
            onScrollFinished({
              // @ts-expect-error TS(2304) FIXME: Cannot find name 'elementsToRender'.
    ...
    handleIntersection = (entries: Array<IntersectionObserverEntry>) => {
      ...
      if (entry.isIntersecting) {
        ...
        if (onScroll) {
          // @ts-expect-error TS(2304) FIXME: Cannot find name 'onScroll'.
