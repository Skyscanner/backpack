...
type RowProps = {
  ...
  selectionConfiguration?: SelectionConfiguration;
  focusedDate: Date | null;
  gridProps: Omit<Props, 'minDate' | 'selectionConfiguration' | 'focusedDate' | 'className' | 'customRowHeight'>;
};
...
const BpkScrollableCalendarGridList = (props: Props) => {
  ...
  useEffect(() => {
    // v2 has no `initialScrollOffset` prop; scroll imperatively after mount.
    ...
    // Mount-only: matches v1 `initialScrollOffset` semantics.
