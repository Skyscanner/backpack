...
describe('withScrimmedPortal', () => {
  it('renders the wrapped component inside a portal correctly with fallback to document.body', () => {
    ...
    expect(
      within(hiddenElements as HTMLElement).queryByText('Wrapped Component'),
    ).toBeNull();
  });
  ...
  it('renders the wrapped component inside a portal with renderTarget provided', () => {
    ...
    expect(document.getElementById('modal-container')).toContainElement(
      screen.getByTestId('dialog-content'),
    );
    expect(
      within(hiddenElements as HTMLElement).queryByText('Wrapped Component'),
  ...
  it('notifies the child component when the portal is ready', () => {
    ...
    expect(
      screen.getByText('Wrapped Component / portal is now ready'),
    ).toBeInTheDocument();
  });
});
...
describe('Server Side Rendering', () => {
  it('renders without crashing', () => {
    const WrappedComponent = () => (
      <div data-testid="dialog-content">Wrapped Component</div>
    );
    const ScrimmedComponent = withScrimmedPortal(WrappedComponent);
