...
# Implementation Plan: BpkInput V2 (Flexible Composable API)

**Package Branch**: `001-bpk-input-v2` | **Date**: 2026-01-29 | **Spec**: [spec.md](./spec.md)
**Input**: Component specification from `/specs/001-bpk-input-v2/spec.md`

**Status**: Phases 1-3 completed. This document updated incrementally based on spec clarifications (2026-01-29).

## Summary

BpkInput V2 implements a flexible composable API inspired by Ark UI, replacing manual docked props with automatic border detection. The component consists of `BpkInput.Root`, `BpkInput.Input`, and `BpkInput.InputAdornment` sub-components. Key clarification from spec: **All InputAdornments (both start and end) must be absolutely positioned at the input element's edges** (start at left edge in LTR / right edge in RTL; end at right edge in LTR / left edge in RTL).

## Spec Clarifications (2026-01-29)

### InputAdornment Absolute Positioning Requirement

**Background**: During spec clarification, it was determined that all InputAdornments must be absolutely positioned to ensure they remain visually connected to the input element.

**Requirements**:
1. **Start InputAdornments**: Absolutely positioned at input's **left edge** (LTR) or **right edge** (RTL)
2. **End InputAdornments**: Absolutely positioned at input's **right edge** (LTR) or **left edge** (RTL)
3. **Multiple End Adornments**: When multiple end adornments exist (user + valid icon + clear button), they stack in order: `[user endAdornment] [valid icon] [clear button]`, all positioned at the same edge
4. **Visual Connection**: Adornments must remain adjacent to input element, not separated by gap

**Implementation** (already completed in Phase 3):
```scss
// All adornments use absolute positioning
...
.bpk-input-v2__adornment--start {
  left: 0;  // Left edge in LTR
  @include utils.bpk-rtl {
    right: 0;  // Right edge in RTL
    left: auto;
  }
}
...
.bpk-input-v2__adornment--end {
  right: 0;  // Right edge in LTR
  @include utils.bpk-rtl {
    left: 0;  // Left edge in RTL
    right: auto;
  }
}
```

**Impact on Gap Calculation**:
- `gap` prop controls spacing between InputAdornment and input **content** (text), not between adornments and input container
- Start adornments: `input.style.textIndent = startAdornmentWidth + gap`
- End adornments: `input.style.paddingRight (LTR) = allEndAdornmentsWidth + gap`
- This ensures text content has correct spacing from adornments while adornments stay anchored to edges
