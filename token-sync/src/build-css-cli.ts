...
// Default layout:
//   token-sync/tokens/   ← Stage 1 DTCG output (regenerated by `npm run tokens:fetch`)
//   token-sync/css/      ← Stage 2 CSS output (regenerated by `npm run tokens:build-css`)
//
// CSS lives outside `tokens/` deliberately: Stage 1's DTCG writer clears the
// whole `tokens/` directory before each sync to drop stale JSON, and we don't
// want that to wipe the committed CSS as a side-effect. Both paths can be
// overridden via env vars to support custom build pipelines.
